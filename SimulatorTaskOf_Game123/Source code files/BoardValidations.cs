...
class BoardValidations
{
    ...
    public virtual byte GetGreatestLengthAtGivenStartPosition(char[,] newBoard, char currentFigure, int row, int col)
    {
        ...
        int countElementsRightDiagonal = analyzeBoard.RightDiagonal(row, col, LIMIT_BOXES, currentFigure).Length;
        int countElementsLeftDiagonal = analyzeBoard.LeftDiagonal(row, col, LIMIT_BOXES, currentFigure).Length;
        int countElementsHorizontal = analyzeBoard.Horizontal(row, col, LIMIT_BOXES, currentFigure).Length;
        int countVertical = analyzeBoard.Vertical(row, col, LIMIT_BOXES, currentFigure).Length;
